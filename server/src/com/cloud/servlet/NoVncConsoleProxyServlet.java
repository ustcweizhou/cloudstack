...
/**
 * Console access : /novncconosole?cmd=access&vm=xxx
 */
...
public class NoVncConsoleProxyServlet extends HttpServlet {
    ...
    private void handleAccessRequest(HttpServletRequest req, HttpServletResponse resp, long vmId) {
        ...
        sb.append("<html><link rel=\"shortcut icon\" href=\"https://www.leaseweb.com/sites/all/themes/leaseweb/favicon.ico\" /><title>").append(ConsoleProxyServlet.escapeHTML(vmName)).append("</title><frameset><frame src=\"").append(composeNoVncConsoleAccessUrl(rootUrl, vm, host, req));
    ...
    private String composeNoVncConsoleAccessUrl(String rootUrl, VirtualMachine vm, HostVO hostVo, HttpServletRequest req) {
        ...
        if (ConsoleProxyManager.NoVncConsolePort.value() != null) {
            sb.append(":" + ConsoleProxyManager.NoVncConsolePort.value() + "/vnc_auto.html?" + encryptor.encrypt(param));
        } else {
            sb.append(":" + ConsoleProxyManager.NoVncConsolePort.defaultValue() + "/vnc_auto.html?" + encryptor.encrypt(param));
        }
